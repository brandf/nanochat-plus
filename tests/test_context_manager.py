...
def test_training_context_manager_builds_masks_and_virtual_flags() -> None:
    config = TrainingContextManagerConfig(
        max_nodes=32,
        tail_token_keep=1,
        holdout_tokens=1,
        virtual_dropout_prob=1.0,
        virtual_dropout_seed=123,
    ...
    assert torch.any(flags & virtual_bit), "Virtual dropout mask should set flag bits."
    assert torch.all((flags & virtual_bit)[holdout] == 0), "Holdout nodes must remain active."


def test_training_virtual_dropout_deterministic_per_seed() -> None:
    base_kwargs = dict(
        max_nodes=32,
        tail_token_keep=0,
        holdout_tokens=0,
        virtual_dropout_prob=0.5,
        gistnet_kwargs=_gistnet_kwargs(),
    )
    latents = _rand_latents(2, 6, base_kwargs["gistnet_kwargs"]["d_model"])

    config_a = TrainingContextManagerConfig(**base_kwargs, virtual_dropout_seed=7)
    config_b = TrainingContextManagerConfig(**base_kwargs, virtual_dropout_seed=7)
    config_c = TrainingContextManagerConfig(**base_kwargs, virtual_dropout_seed=11)

    mask_a = TrainingContextManager(config_a).build_batch(latents)["virtual_mask"]
    mask_b = TrainingContextManager(config_b).build_batch(latents)["virtual_mask"]
    mask_c = TrainingContextManager(config_c).build_batch(latents)["virtual_mask"]

    assert torch.equal(mask_a, mask_b), "Seeds must produce deterministic virtual masks."
    assert not torch.equal(mask_a, mask_c), "Different seeds should decorrelate dropout masks."


def test_training_holdout_resists_virtual_dropout() -> None:
    config = TrainingContextManagerConfig(
        max_nodes=32,
        tail_token_keep=0,
        holdout_tokens=2,
    ...
    assert torch.all((flags & virtual_bit)[holdout_mask] == 0)
...
def test_inference_manager_refocus_respects_tail_window() -> None:
    ...
    assert torch.all((flags[0, keep] & virtual_bit) == 0), "Tail tokens must never collapse."
...
def test_inference_force_refocus_uses_focus_override() -> None:
    ...
    live_tokens = int(((flags[0, :active] & virtual_bit) == 0)[token_mask].sum().item())
...
def test_inference_multi_batch_refocus_independent() -> None:
    ...
    for b in range(2):
        ...
        live_tokens = int(((flags[b, :active] & virtual_bit) == 0)[token_mask].sum().item())
        assert live_tokens <= config.target_node_count
